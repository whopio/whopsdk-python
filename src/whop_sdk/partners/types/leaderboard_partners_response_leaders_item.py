# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .leaderboard_partners_response_leaders_item_user import LeaderboardPartnersResponseLeadersItemUser


class LeaderboardPartnersResponseLeadersItem(UniversalBaseModel):
    first_referral_started_at: dt.datetime = pydantic.Field()
    """
    When the referrer's earliest partner business became active.
    """

    rank: int = pydantic.Field()
    """
    1-based leaderboard position.
    """

    total_earnings_usd: str = pydantic.Field()
    """
    The referrer's pending + completed earnings across all referred businesses, in USD.
    """

    total_volume_usd: str = pydantic.Field()
    """
    Credited GMV across all the referrer's referred businesses, in USD.
    """

    user: typing.Optional[LeaderboardPartnersResponseLeadersItemUser] = pydantic.Field(default=None)
    """
    The ranked referrer. Identity fields (id, name, username, profile_picture) are returned only on the caller's own entry; other referrers expose coarse location only.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
