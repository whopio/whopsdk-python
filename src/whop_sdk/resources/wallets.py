...
class WalletsResource(SyncAPIResource):
    ...
    def balance(
        ...
        *,
        ...
        """
        Returns the account's balance summary — available (card spending power),
        pending, and total in USD — plus the crypto held in each of the account's crypto
        wallets, as Fragment's ledger mirrors it: one row per crypto wallet / chain /
        token, with its USD value.
