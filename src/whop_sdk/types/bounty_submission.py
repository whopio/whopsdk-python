...
class BountySubmission(BaseModel):
    ...
    status: Optional[Literal["in_progress", "submitted", "approved", "denied"]] = None
    """Lifecycle state.
    ...
    paid; `denied` submissions were rejected. `null` when the attempt ended without
    proof, taking it out of the public lifecycle — those attempts are absent from
    every public list and read.
    """
