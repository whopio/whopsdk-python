# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ....core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .create_direct_request_plan_product_global_affiliate_status import (
    CreateDirectRequestPlanProductGlobalAffiliateStatus,
)
from .create_direct_request_plan_product_visibility import CreateDirectRequestPlanProductVisibility


class CreateDirectRequestPlanProduct(UniversalBaseModel):
    """
    Find or create a product by external identifier. Mutually exclusive with product_id.
    """

    collect_shipping_address: typing.Optional[bool] = pydantic.Field(default=None)
    """
    Whether to collect a shipping address at checkout.
    """

    custom_statement_descriptor: typing.Optional[str] = pydantic.Field(default=None)
    """
    Custom card statement descriptor for the product, starting with WHOP*.
    """

    description: typing.Optional[str] = pydantic.Field(default=None)
    """
    Product description.
    """

    external_identifier: str = pydantic.Field()
    """
    Your unique identifier for the product.
    """

    global_affiliate_percentage: typing.Optional[float] = pydantic.Field(default=None)
    """
    Percentage of revenue paid to global affiliates.
    """

    global_affiliate_status: typing.Optional[CreateDirectRequestPlanProductGlobalAffiliateStatus] = pydantic.Field(
        default=None
    )
    """
    Global affiliate program status.
    """

    headline: typing.Optional[str] = pydantic.Field(default=None)
    """
    Product headline.
    """

    product_tax_code_id: typing.Optional[str] = pydantic.Field(default=None)
    """
    Product tax code identifier.
    """

    redirect_purchase_url: typing.Optional[str] = pydantic.Field(default=None)
    """
    Where to redirect the buyer after purchase.
    """

    route: typing.Optional[str] = pydantic.Field(default=None)
    """
    Product route.
    """

    title: str = pydantic.Field()
    """
    Product title.
    """

    visibility: typing.Optional[CreateDirectRequestPlanProductVisibility] = pydantic.Field(default=None)
    """
    Product visibility. Defaults to hidden.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
