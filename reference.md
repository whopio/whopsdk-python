...
**blueprint_id:** `typing.Optional[str]` — The blueprint App ID, prefixed `app_`. Creates a hosted website for the account and queues its deployment asynchronously; the Account response does not report deployment completion.
...
<details><summary><code>client.accounts.<a href="src/whop_sdk/accounts/client.py">delete</a>(...) -> DeleteAccountsResponse</code></summary>
<dl>
<dd>
...
**dynamic_creative:** `typing.Optional[bool]` — Whether the ad platform automatically mixes and matches this ad group's creatives and copy to find the best-performing combinations. Meta has deprecated it and rejects `true`; ad groups already using it keep it, and it can't be changed afterward.
...
**verified:** `typing.Optional[bool]` — Only return apps whose Whop verification status matches this value. Omit this filter to include every verification status the caller can see.
...
**verified_apps_only:** `typing.Optional[bool]` — Legacy compatibility filter. Use `verified` for field equality. `true` returns verified apps; clients pinned before `2026-08-25-2` retain the earlier public website discovery behavior.
...
**recommended:** `typing.Optional[bool]` — Only return apps Whop recommends (or, with `false`, only those it does not), independently of verification status.
...
**merchant_category_code:** `typing.Optional[str]` — Four-digit MCC, including leading zeros. Null matches any MCC. When both merchant filters are absent, scoped_account_id is required.
...
**merchant_name:** `typing.Optional[str]` — Raw merchant name reported by the card provider, not the enriched display name. Omit or set null to match any merchant name. Supplied names must contain a non-whitespace character and match together with any MCC filter.
...
**scoped_account_id:** `typing.Optional[str]` — Account ID prefixed biz_ belonging to a direct connected account. Required when both merchant filters are omitted or null. Otherwise, omit or set null to designate all direct connected accounts.
...
<details><summary><code>client.cashback_rules.<a href="src/whop_sdk/cashback_rules/client.py">payout</a>(...) -> CashbackPayout</code></summary>
<dl>
<dd>
...
**account_id:** `typing.Optional[str]` — Pay only this direct connected account.
...
**cashback_rule_id:** `typing.Optional[str]` — Pay only transactions whose winning cashback rule has this ID and is funded by the authenticated platform.
...
**transaction_id:** `typing.Optional[str]` — Pay only this card transaction belonging to a direct connected account.
...
<details><summary><code>client.cashback_rules.<a href="src/whop_sdk/cashback_rules/client.py">update</a>(...) -> CashbackRule</code></summary>
<dl>
<dd>
...
**id:** `str` — ID of the cashback rule, prefixed cicbr_.
...
**description:** `typing.Optional[str]` — Description of the rule. Set null to clear it.
...
**expires_at:** `typing.Optional[datetime.datetime]` — Exclusive end as an ISO 8601 timestamp, strictly later than the original starts_at. May be in the past to end an active rule. Set null to remove the expiration.
...
**merchant_name:** `typing.Optional[str]` — Raw merchant name reported by the card provider. Set null to match any merchant name. Supplied names must contain a non-whitespace character and match together with any MCC filter. Clearing both filters requires an existing scoped_account_id.
...
**status:** `typing.Optional[typing.Union[ListDisputesRequestStatusItem, typing.Sequence[ListDisputesRequestStatusItem]]]` — Only disputes in these statuses. Repeat the parameter to pass several — one paginated list covers all of them. Inquiries match only the `warning_` statuses. A `needs_response` dispute whose evidence deadline has passed reports and filters as `under_review` instead.
...
<details><summary><code>client.economic_intelligence.<a href="src/whop_sdk/economic_intelligence/client.py">list</a>(...) -> ListEconomicIntelligenceResponse</code></summary>
<dl>
<dd>
...
**app_build_id:** `typing.Optional[str]` — The build of the hosted app that served the page where the event occurred.
...
**app_id:** `typing.Optional[str]` — The hosted app that served the page where the event occurred.
...
## Experiments
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">list</a>(...) -> ListExperimentsResponse</code></summary>
<dl>
<dd>
...
**related_resource:** `typing.Optional[ExperimentResourceReference]` — Filter by related resource; requires account_id.
...
**status:** `typing.Optional[ListExperimentsRequestStatus]` — Only experiments with this status.
...
**order:** `typing.Optional[ListExperimentsRequestOrder]` — The field to sort experiments by.
...
**direction:** `typing.Optional[ListExperimentsRequestDirection]` — Sort direction.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">create</a>(...) -> Experiment</code></summary>
<dl>
<dd>
...
**flag_key:** `str` — Developer-chosen handle referenced from code, used in evaluation and results calls. Interchangeable with the returned `expt_` id anywhere the API takes an experiment identifier.
...
**feature_flag_only:** `typing.Optional[bool]` — When `true`, creates a binary feature flag rather than a full experiment. Feature flags expose the same evaluation API but do not collect metric results. Defaults to `false`.
...
**name:** `typing.Optional[str]` — Human-readable display name. Defaults to `flag_key` when omitted.
...
**targeting_rules:** `typing.Optional[typing.List[CreateExperimentsRequestTargetingRulesItem]]` — Rules that determine which subjects qualify for the experiment.
...
**variants:** `typing.Optional[typing.List[CreateExperimentsRequestVariantsItem]]` — Treatment arms to create. Users outside every arm form the implicit `control` group. Required unless `feature_flag_only` is `true`.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">exposures</a>(...) -> ExposuresExperimentsResponse</code></summary>
<dl>
<dd>
...
**related_resource:** `typing.Optional[ExperimentResourceReference]` — Restricts batch evaluation to this related resource; omitted batches contain only unbound experiments.
...
**flag_key:** `typing.Optional[str]` — Flag or experiment to evaluate — the flag_key handle or the `expt_` id. Omit to return all flags the caller qualifies for.
...
**log_exposure:** `typing.Optional[bool]` — Set false to evaluate without recording an exposure. Omitted records it.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">retrieve</a>(...) -> Experiment</code></summary>
<dl>
<dd>
...
**id:** `str` — The experiment identifier — the `expt_` id or the flag_key handle.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">update</a>(...) -> Experiment</code></summary>
<dl>
<dd>
...
**targeting_rules:** `typing.Optional[typing.List[UpdateExperimentsRequestTargetingRulesItem]]` — Replace the targeting rules with this set. Omit to leave unchanged.
...
**variants:** `typing.Optional[typing.List[UpdateExperimentsRequestVariantsItem]]` — Grow treatment allocation. Pass every existing treatment with an equal-or-higher weight; append new names to add arms. Weights never decrease and arms are never removed. Omit to leave unchanged.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">activate</a>(...) -> Experiment</code></summary>
<dl>
<dd>
...
**duration_days:** `typing.Optional[int]` — Reporting window length in days. Omit to use the default. This does not automatically end the experiment.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">end</a>(...) -> Experiment</code></summary>
<dl>
<dd>
...
**findings:** `str` — What you learned and why you chose this outcome. Required.
...
<details><summary><code>client.experiments.<a href="src/whop_sdk/experiments/client.py">pause</a>(...) -> Experiment</code></summary>
<dl>
<dd>
...
## FinancialReports
<details><summary><code>client.financial_reports.<a href="src/whop_sdk/financial_reports/client.py">retrieve</a>(...) -> RetrieveFinancialReportsResponse</code></summary>
<dl>
<dd>
...
**from:** `typing.Optional[datetime.datetime]` — Start of the report window as an ISO 8601 timestamp. Required for platform-wide (global) reports.
...
**to:** `typing.Optional[datetime.datetime]` — Exclusive end of the report window as an ISO 8601 timestamp. Required for platform-wide (global) reports.
...
**timezone:** `typing.Optional[str]` — IANA timezone (for example `America/New_York`) used to bucket report periods. Defaults to UTC. `from` and `to` remain exact instants.
...
**line_types:** `typing.Optional[typing.Union[RetrieveFinancialReportsRequestLineTypesItem, typing.Sequence[RetrieveFinancialReportsRequestLineTypesItem]]]` — Account-level balance activity only: ledger line categories to include.
...
**direction:** `typing.Optional[RetrieveFinancialReportsRequestDirection]` — Account-level balance activity only: include money moving in or money moving out.
...
**cumulative:** `typing.Optional[bool]` — Platform-wide (global) reports only: when true, return cumulative balances as of to (all history, no lower bound) instead of activity within the period.
...
**include_payment_fee_breakdown:** `typing.Optional[bool]` — Balance activity only: include payment costs grouped by payment method and provider.
...
<details><summary><code>client.memberships.<a href="src/whop_sdk/memberships/client.py">resync_access</a>(...) -> Membership</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="src/whop_sdk/partner_referral_requests/client.py">list</a>(...) -> ListPartnerReferralRequestsResponse</code></summary>
<dl>
<dd>
...
**account_id:** `typing.Optional[str]` — Only requests for this business ID, prefixed `biz_`.
...
**target_user_id:** `typing.Optional[str]` — Only requests received by this user ID, prefixed `user_`.
...
**partner_id:** `typing.Optional[str]` — Only requests sent by this partner's user ID, prefixed `user_`.
...
**status:** `typing.Optional[ListPartnerReferralRequestsRequestStatus]` — Only requests with this approval status.
...
**request_type:** `typing.Optional[ListPartnerReferralRequestsRequestRequestType]` — Only requests initiated in this way.
...
**order:** `typing.Optional[ListPartnerReferralRequestsRequestOrder]` — Field used to sort requests.
...
**direction:** `typing.Optional[ListPartnerReferralRequestsRequestDirection]` — Sort direction.
...
<details><summary><code>client.partner_referral_requests.<a href="src/whop_sdk/partner_referral_requests/client.py">create</a>(...) -> PartnerReferralRequest</code></summary>
<dl>
<dd>
...
from whop_sdk import Whop
from whop_sdk.environment import WhopEnvironment
from whop_sdk.partner_referral_requests import CreatePartnerReferralRequestsRequestBodyAccountId
...
<details><summary><code>client.partner_referral_requests.<a href="src/whop_sdk/partner_referral_requests/client.py">retrieve</a>(...) -> PartnerReferralRequest</code></summary>
<dl>
<dd>
...
**id:** `str` — Partner referral request ID, prefixed `prfr_`.
...
<details><summary><code>client.partner_referral_requests.<a href="src/whop_sdk/partner_referral_requests/client.py">accept</a>(...) -> PartnerReferralRequest</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="src/whop_sdk/partner_referral_requests/client.py">cancel</a>(...) -> PartnerReferralRequest</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partner_referral_requests.<a href="src/whop_sdk/partner_referral_requests/client.py">decline</a>(...) -> PartnerReferralRequest</code></summary>
<dl>
<dd>
...
<details><summary><code>client.partners.<a href="src/whop_sdk/partners/client.py">retrieve</a>(...) -> Partner</code></summary>
<dl>
<dd>
...
**id:** `str` — The authenticated partner's user ID, prefixed user_, or me. Other users' profiles are not accessible.
...
**account_id:** `typing.Optional[str]` — Only payments charged by this account, prefixed `biz_`.
...
**status:** `typing.Optional[ListPaymentsRequestStatus]` — Only payments in this lifecycle state.
...
**billing_reason:** `typing.Optional[ListPaymentsRequestBillingReason]` — Only payments charged for this reason.
...
**currency:** `typing.Optional[str]` — Only payments presented in this three-letter currency, such as `usd`.
...
**created_before:** `typing.Optional[datetime.datetime]` — Only payments created before this ISO 8601 timestamp.
...
**created_after:** `typing.Optional[datetime.datetime]` — Only payments created after this ISO 8601 timestamp.
...
**order:** `typing.Optional[ListPaymentsRequestOrder]` — The field to sort by.
...
**direction:** `typing.Optional[ListPaymentsRequestDirection]` — The sort direction.
...
<details><summary><code>client.payments.<a href="src/whop_sdk/payments/client.py">create</a>(...) -> Payment</code></summary>
<dl>
<dd>
...
**member_id:** `typing.Optional[str]` — The member to charge, prefixed `mber_`. Required with `payment_method_id` unless `confirmation_token` is provided.
...
**return_url:** `typing.Optional[str]` — Where the buyer continues after completing an off-site step. An absolute https URL without credentials, at most 2,048 characters. Ignored unless `confirmation_token` is provided.
...
<details><summary><code>client.payments.<a href="src/whop_sdk/payments/client.py">retrieve</a>(...) -> Payment</code></summary>
<dl>
<dd>
...
<details><summary><code>client.payments.<a href="src/whop_sdk/payments/client.py">capture</a>(...) -> PaymentStatus</code></summary>
<dl>
<dd>
...
**id:** `str` — The payment whose fees to list, prefixed `pay_`.
...
**id:** `str` — The payment to refund, prefixed `pay_`.
...
**id:** `str` — The payment to retry, prefixed `pay_`.
...
**id:** `str` — The payment to void, prefixed `pay_`.
...
<details><summary><code>client.payments.<a href="src/whop_sdk/payments/client.py">resume</a>(...) -> PaymentStatus</code></summary>
<dl>
<dd>
...
**amount:** `float` — The amount to pay out in the specified currency.
...
<details><summary><code>client.payouts.<a href="src/whop_sdk/payouts/client.py">create_quote</a>(...) -> CreateQuotePayoutsResponse</code></summary>
<dl>
<dd>
...
**payout_method_id:** `str` — The saved payout method to quote (a potk_ identifier).
...
**currency:** `typing.Optional[str]` — The currency to pay out. When omitted, uses `usd` if that balance can cover a withdrawal, otherwise the account's only other funded currency.
...
**platform_covers_fees:** `typing.Optional[bool]` — Whether the parent platform covers the payout fee instead of the account being paid out.
...
**speed:** `typing.Optional[CreateQuotePayoutsRequestSpeed]` — How fast the funds should arrive.
...
**account_id:** `typing.Optional[str]` — Only refunds issued by this account, prefixed `biz_`.
...
**payment_id:** `typing.Optional[str]` — Only refunds of this payment, prefixed `pay_`.
...
**user_id:** `typing.Optional[str]` — Only refunds to this buyer, prefixed `user_`.
...
**created_before:** `typing.Optional[datetime.datetime]` — Only refunds requested before this ISO 8601 timestamp.
...
**created_after:** `typing.Optional[datetime.datetime]` — Only refunds requested after this ISO 8601 timestamp.
...
**order:** `typing.Optional[ListRefundsRequestOrder]` — The field to sort by.
...
**direction:** `typing.Optional[ListRefundsRequestDirection]` — The sort direction.
...
**id:** `str` — The refund to retrieve, prefixed `rf_`.
...
**account_id:** `typing.Optional[str]` — The Account (biz_ identifier) to connect the social account for. An account-scoped API key may omit this to default to its own account. Omit for user profile connections.
...
**interval:** `typing.Optional[RetrieveStatsRequestInterval]` — How wide each point is. Defaults to day. Snapshot metrics are day-only. Funnels support at most 2,000 first-entry cohort buckets.
...
**steps:** `typing.Optional[RetrieveStatsRequestSteps]` — Funnel only. Required when metric=funnel. Consecutive one-based steps encoded as steps[1][event], steps[1][page], steps[2][event], and so on. Values are scalar strings, never JSON.
...
**covered_payout_fees:** `typing.Optional[UpdateFeesRequestCoveredPayoutFees]` — Changes to the payout fees this account covers for connected accounts. Send either all or individual category keys. Omitted categories stay unchanged; category changes have no effect while all is true.
...
**ads_reporting_currency:** `typing.Optional[str]` — Lowercase ISO currency code, such as `usd` or `eur`, used to display ad spend and stats. Defaults to `usd`. Requires the `ad_campaign:create` scope on your API key.
...
**ads_scheduling_timezone:** `typing.Optional[str]` — IANA timezone (e.g. `America/New_York`) used to interpret campaign start/end times and to bucket reports. Cannot be cleared once set — pass a new value to change it. Requires the `ad_campaign:create` scope on your API key.
...
**cards_auto_top_up:** `typing.Optional[bool]` — Whether incoming funds are automatically moved to the account's cards balance. Requires a cards balance on the account and the `payout:account:update` scope on your API key.
...
**dispute_fighter_enabled:** `typing.Optional[bool]` — Whether Whop assembles and files the evidence response when this account's payments are disputed. Off by default; enabling it also opts the account into the success fee charged only on disputes it wins. Requires the `payment:dispute` scope on your API key.
...
**subscription_failure_behavior:** `typing.Optional[UpdatePreferencesRequestSubscriptionFailureBehavior]` — What happens to a subscription once every retry of a renewal payment has failed. `cancel` (the default) cancels it. `none` leaves it past due and keeps billing it each period; access follows the account's past-due access setting. Requires company:manage_checkout permission.
...
**from:** `datetime.datetime` — Start of the report window as an ISO 8601 timestamp.
...
**timezone:** `typing.Optional[str]` — IANA timezone used by the parent report to bucket periods. Defaults to UTC.
...
**tier:** `typing.Optional[ListBusinessesRequestTier]` — Filter to referrals from a single tier: first, second, or blueprint.
